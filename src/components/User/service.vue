...
export default {
  ...
  data() {
    return {
      ...
      form: { // 发送数据提交数据表单
        contents: ""
      },servicelist:['何江','admin'],
      res1:{
        "flag":true,
        "total": 1,
        "data": [{
          "time": "2020-12-16 15:23:27", // 系统响应时间
          "author": "true",  // 用户头像
          "content": "你好，我是test", // 消息内容
          "index": 0, // 消息ID，用来去重
          "consumer": "碧海燕鱼", // 用户昵称
          "manager": "客服1" // 用户编码
        }, {
  ...
  methods: {
    ...
    sendMsg() { // 发送消息
      ...
      let manager_Arg=Math.floor(Math.random()*this.servicelist.length);
      let manager=this.servicelist[manager_Arg]
      for (let i=0;i<this.data.length;i++){
        if (this.data[i].manager!=null){
          manager=this.data[i].manager;
          break;
        }
      }
      this.data=[]
      axios.get("http://" + sessionStorage.getItem('ip') + ":8080/user/service/"+this.state1.user.name+"/"+this.servicelist[0]+"/"+
    ...
    reloadData(){
      if(this.timerSwitch){
          ...
          //listMsgByOrder
          this.data=[]
          axios.put("http://"+sessionStorage.getItem('ip')+":8080/user/service/consumerHistory",{consumer:this.state1.user.name}).then(
            (res)=>{
              if (res.data) { //有返回
                res.data.data.forEach(e => {
                  // 标记最大偏移位
